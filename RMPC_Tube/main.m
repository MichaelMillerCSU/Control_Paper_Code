...
for i = 1 : step_Max
    ...
    U_Record = [U_Record, u_Designed];
    V_Record = [V_Record, v_Solved];
end

P_X_Record = plot(X_Record(1, :), X_Record(2, :), 'color', [0 0.4470 0.7410], 'LineWidth', 2, 'LineStyle', '--', 'Marker', "*", 'MarkerSize', 5);
hold on
P_X0_Record = plot(X0_Record(1, :), X0_Record(2, :), 'color', [0.8500 0.3250 0.0980], 'LineWidth', 2, 'LineStyle', '-', 'Marker', "o", 'MarkerSize', 5);
hold on
P_Xf = plot_Set(Xf, [0.9290 0.6940 0.1250], 3);
hold on
P_Xf_Z = plot_Set(Xf + Z, [0.4940 0.1840 0.5560], 3);
hold on
P_Xc = plot_Set(X_Constraints, [0.6350 0.0780 0.1840], 3);
hold on
P_Xc_Z = plot_Set(X_Constraints - Z, [0.3010 0.7450 0.9330], 3);
legend([P_Z P_X_Record P_X0_Record P_Xf P_Xf_Z P_Xc P_Xc_Z], {'$\mathcal{Z}$', '$Real\,state\,x$', '$Nominal\,state\,\bar{x}$', '$X_f$', '$X_f\oplus\mathcal{Z}$', '$X_c$', '$X_c\ominus\mathcal{Z}$'}, 'Interpreter','latex')
grid on 
axis([-10 4 -3 2])
...
figure
P_U_Record = plot(U_Record, 'color', [0 0.4470 0.7410], 'LineWidth', 2, 'LineStyle', '-');
hold on 
P_V_Record = plot(V_Record, 'color', [0.8500 0.3250 0.0980], 'LineWidth', 2, 'LineStyle', '-');
legend([P_U_Record P_V_Record], {'$u(t)$', '$v(t)$'}, 'Interpreter','latex')
grid on 
xlabel('$Steps$', 'Interpreter','latex')
ylabel('$u(t)/v(t)$', 'Interpreter','latex')
